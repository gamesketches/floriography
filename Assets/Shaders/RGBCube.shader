Shader "Tutorial/Cg shader for RGB cube" { 
   SubShader { 
      Pass { 
         CGPROGRAM 
            ...
            // vertex shader 
         {
            ...
            output.col = float4((vertexPos.x + vertexPos.y + vertexPos.z)/3 + 0.5, 
            					(vertexPos.x + vertexPos.y + vertexPos.z)/3 + 0.5,
            					(vertexPos.x + vertexPos.y + vertexPos.z)/3 + 0.5,
            					0.0);//vertexPos + float4(0.5, 0.5, 0.5, 0.0));
